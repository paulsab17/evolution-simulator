...
fluidRow(
  column(6,
         sliderInput("pA0_1", label = "p(A) in Pop 1:",
              min = 0, max = 1, value = 0.6, step = 0.001,width = "auto")),
  column(6,
         sliderInput("pA0_2", label = "p(A) in Pop 2:",
              min = 0, max = 1, value = 0.4, step = 0.001,width = "auto"))
)
fluidRow(
  column(6,
...
renderPlot({
  ...
  ggplot(gen_table_2,aes(x = Gen,y = pA,color = factor(pop),group = factor(pop))) +
    geom_line() + 
    ylim(0,1) +
    labs(y = "p(A)",x="Generation") +
